...
## 2026-07-25 - [Focus Loss & Overlap Prevention in Floating Anchors]
**Learning:** Floating "Back to Top" anchors can lead to critical accessibility and collision issues if focus transitions and layout offsets are not carefully synchronized. Moving the anchor dynamically out of the tab index (`tabindex="-1"`, `aria-hidden="true"`) when hidden prevents keyboard traps on invisible elements. Once activated, clicking it must programmatically redirect keyboard focus to the skip-to-content link at the top, preventing focus loss when the button hides itself again. Furthermore, adjacent dynamic alerts (like Toasts) must adjust their baseline offset (e.g., from `bottom: 2rem` to `6rem`) to prevent overlapping visual blocks during multi-state user flows.
**Action:** Always pair floating buttons with programmatic focus redirection, dynamic tabindex states, and responsive positioning offsets for concurrent notification overlays.
...
## 2026-07-31 - [Custom Validation Interception & ARIA Invalid States]
**Learning:** Real-time custom input validation can easily fall out of sync with browser-native form submission pipelines. Even with explicit error messages and validation classes, if the custom logic is more strict than standard browser formats (e.g., rejecting single-part domains allowed by browser-native `type="email"` validation), forms can still submit invalid data and trigger misleading success animations. Intercepting the submit event to explicitly validate inputs, programmatically redirect keyboard focus, and update `aria-invalid` attributes is essential to prevent successful submission of invalid form states and maintain accessibility synchronization.
**Action:** Always intercept form submission handlers to block completion when custom inputs fail validation, programmatically focus the invalid element, and dynamically synchronize `aria-invalid` states during both live entries and submission cycles.
