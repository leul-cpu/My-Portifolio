...
## 2026-08-01 - [Real-Time Assistive Submission Validation]
**Learning:** Performing real-time email validation is helpful, but if the form submit handler doesn't block submission of invalid inputs, users can bypass the custom rules (especially when custom regex validation is stricter than lax native browser type checks). Intercepting the submission, focusing the invalid field, triggering a transient accessible toast, and synchronizing dynamic `aria-invalid` attributes prevents bad submissions and ensures assistive technologies are immediately notified of the corrective action needed.
**Action:** Always intercept submissions to validate critical fields against custom rules, redirect focus to invalid inputs, update `aria-invalid` states dynamically, and provide non-modal status announcements for validation outcomes.
