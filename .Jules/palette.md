...
## 2026-07-24 - [Focus Redirection & Overlap Prevention]
**Learning:** Floating utility controls like "Back to Top" buttons introduce subtle keyboard traps and layout overlaps. If a keyboard user triggers a "Back to Top" button that fades out of view at the top of the page, focus is lost on a hidden element. Programmatically redirecting focus to the top skip-link or logo restores tab index sanity. Furthermore, to avoid visual collision, temporary toast notification positions must be adjusted (e.g., from `bottom: 2rem` to `bottom: 6rem`) to co-exist nicely with the floating button's footprint.
**Action:** Always redirect focus to the top of the page when clicking back-to-top buttons, and check layout bounding boxes of floating utilities to avoid stacking collisions.
