...
## 2026-07-21 - [Keyboard Shortcut Hints & Temporary ARIA Success States]
**Learning:** Adding keyboard shortcuts (e.g., "Press T" to toggle theme) significantly empowers power-users, but they remain undiscoverable unless explicitly advertised via dynamic `title` and `aria-label` attributes. Furthermore, static clipboard copy buttons lack post-action tactile or contextual clarity. Temporarily updating the clipboard button's ARIA attributes and native title (e.g., to "Email address copied!" for 3 seconds) creates a robust dual-channel (visual & assistive) confirmation loop that guarantees accessibility and prevents cognitive friction.
**Action:** Always pair power-user keyboard shortcuts with clear tooltips/labels, and temporarily update critical screen reader tags (like `aria-label`) for transient state changes.
